...
const updateSemesterRegistrationIntoDB = async (
  ...
  if (currentStatus === RegistrationStatus.ENDED)
    throw new AppError(
      httpStatus.BAD_REQUEST,
      `This semester is already ${currentStatus}`,
    );
  ...
  if (
    ...
    throw new AppError(httpStatus.BAD_REQUEST, invalidTransitionMsg);
...
const deleteSemesterRegistrationFromDB = async (id: string) => {
  ...
  if (!isSemesterRegistrationExists)
    throw new AppError(httpStatus.NOT_FOUND, 'This registered semester is not found !');
  ...
  try {
    ...
    const deletedSemesterRegistration = await SemesterRegistrationModel.findByIdAndDelete(
      id,
      { new: true },
    ).session(deleteSession);
    ...
    throw new AppError(
      ...
      error.message,
      'An error occurred while deleting semester registration',
    );
  } finally {
